...
import { captureFocus, restoreFocus } from "../../helpers/focus.js";
  ...
  function renderSession() {
    if (session.completed) {
      return el("div", { class: classes("root", "completion") }, [
        el("h2", { class: ui.title, text: "Survey complete" }),
        el("p", { class: ui.description, text: "Thank you for your response." })
      ]);
    }
    ...
    return el("form", {
      ...
      el("section", { class: ui.page }, [
        el("header", { class: ui.header }, [
          el("p", {
            class: ui.eyebrow,
            text: `Page ${session.pageIndex + 1} of ${session.survey.pages.length}`
          }),
        ...
        questions.length
          ...
          : el("p", { class: ui.empty, text: "No visible questions on this page." }),
        el("div", { class: ui.actions }, [
          ...
          el("button", {
            ...
            type: "submit",
            text: session.pageIndex === session.survey.pages.length - 1 ? "Submit" : "Next"
          })
        ])
      ])
    ]);
  ...
  function renderPanel(question) {
    ...
    return el("fieldset", { class: `${ui.question} ${ui.panel}`, "data-question-name": question.name }, [
      ...
      visibleNestedQuestions.length
        ...
        : el("p", { class: ui.empty, text: "No visible questions in this panel." })
    ]);
  }
